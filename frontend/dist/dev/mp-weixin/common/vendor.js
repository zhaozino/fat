    ...
    index.request({
      ...
      success(res) {
        ...
        if (res.statusCode === 401) {
          index.removeStorageSync("token");
          index.reLaunch({ url: "/src/pages/login/index" });
          return reject(new Error("未登录"));
        }
        if (res.statusCode >= 400) {
          const msg = ((_a = res.data) == null ? void 0 : _a.message) || "请求失败";
          index.showToast({ title: msg, icon: "none" });
          return reject(new Error(msg));
        }
        resolve2(res.data);
      },
      fail(err) {
        index.showToast({ title: "网络异常，请重试", icon: "none" });
        reject(err);
      }
    });
  });
...
const useUserStore = /* @__PURE__ */ defineStore("user", () => {
  const token = ref(index.getStorageSync("token") || "");
  const profile = ref(null);
  const todayRecord = ref(null);
  const isLoggedIn = computed(() => !!token.value);
  const bmr = computed(() => {
    var _a;
    return ((_a = profile.value) == null ? void 0 : _a.bmr) || 0;
  });
  const targetDeficit = computed(() => {
    var _a;
    return ((_a = profile.value) == null ? void 0 : _a.targetDeficit) || 400;
  });
  function checkAuth() {
    if (!token.value) {
      index.reLaunch({ url: "/src/pages/login/index" });
...
exports.e = e;
exports.encourageApi = encourageApi;
exports.estimateWeightLoss = estimateWeightLoss;
exports.f = f;
