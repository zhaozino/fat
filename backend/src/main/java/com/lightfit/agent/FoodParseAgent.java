...
public class FoodParseAgent {
    ...
    private static final String SYSTEM_PROMPT = """
            ...
            热量计算要求：
            - calories 字段必须是用户实际吃的总热量，不是每100g的热量
            - 计算步骤：先估算实际食用重量（g），再乘以该食物每克热量
            - 注意区分"每100g热量"和"一份/一个的实际热量"，例如鸡蛋每100g约155大卡，但一个鸡蛋约50g所以约78大卡
            - 先确定食物的可食用部分重量（如西瓜去皮约占总重60%），再按每100g可食用部分的热量来计算
            - 常见参考：米饭 116kcal/100g（一碗约150g），西瓜（可食部分）26kcal/100g，苹果 52kcal/100g（一个约200g），鸡蛋 155kcal/100g（一个约50g）
            - 1斤=500g，注意区分带皮重量和可食用重量
